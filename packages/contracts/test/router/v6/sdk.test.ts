...
describe("[ReservoirV6_0_0] Filling listings and bids via the SDK", () => {
  ...
  it("Fill multiple cross-currency Seaport listings with usdc", async () => {
    ...
    async function swapUSDC() {
      const router2 = new Sdk.RouterV6.Router(chainId, ethers.provider);
      const swapExecutions = [
        // 1. Swap ETH for USDC on UniswapV3
        {
          module: router2.contracts.uniswapV3Module.address,
          data: router2.contracts.uniswapV3Module.interface.encodeFunctionData("ethToExactOutput", [
            {
      ...
      // Swap to erc20
      try {
        await router2.contracts.router.connect(dan).execute(swapExecutions, {
    ...
    console.log('permits', tx.permits)
    ...
    for (const permit of tx.permits) {
      ...
      console.log("transferDetails", permit.details.data.transferDetails[0].amount.toString())
    }
    ...
    const txCall = await buyer.sendTransaction(txData);
    const recipient = await txCall.wait();
    console.log(recipient.logs);

    for (const log of recipient.logs) {
      try {
        const parsed = permit2Module.interface.parseLog(log);
    ...
    const permitAllowance = await permitHandler.permit2.allowance(
      buyer.address, 
      Sdk.Common.Addresses.Usdc[chainId], 
      permitHandler.module
    )

    console.log(
      'permit',
      permitAllowance,
      permitAllowance.amount.toString(),
      {
        permit2: permitHandler.permit2.address,
        sender: buyer.address, 
        token: Sdk.Common.Addresses.Usdc[chainId], 
        module: permitHandler.module
      }
    )
